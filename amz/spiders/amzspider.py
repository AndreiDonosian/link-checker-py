...
class AmzspiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        if(self._connect()):
            ...
            for urls in self.cursor.fetchall():
                try:
                    yield scrapy.Request(str(urls[1]), self.parse, errback=self.err_parse) #link attr
                except:
    ...
    def parse(self, response):
        ...
        if(self.cursor_insert):
            ...
            if(asin is not None):
                ...
                alter_images = self.html_escape(str(self._alter_images(asin,response)))
                ...
                if('tech_date_first_available'  in tech_part.keys()):                
                    tech_date_first_available = datetime.datetime.strptime(tech_part['tech_date_first_available'], "%B %d, %Y").date()
                ...
                self.cursor_insert.execute("UPDATE to_parse set today_parsed=1, last_parsed=NOW(), asin = '%s' WHERE link='%s'" %(asin,str(response.url)))

                self.db.commit()
            else:
                print("No valiable asin")
    ...
    def _alter_images(self,asin, response):
        filepath = "/var/www/html/basic/web/downloaded_content/"+asin
        if(os.path.exists(filepath)):
            print("")
        else:
            os.mkdir("/var/www/html/basic/web/downloaded_content/"+asin)

        counter = 0
        imagenames = []
        for z in response.css('#altImages img::attr(src)').getall():
            filenamez = filepath+"/"+str(counter)+"."+z.split(".")[-1]
            urllib.urlretrieve(z, filenamez)
            counter+=1
            # os.system("curl -O "+z+" /var/www/html/basic/web/downloaded_content/"+asin+"/"+counter+"."+z.split(".")[-1])
            # imagenames.append(asin+"/"+counter+"."+z.split(".")[-1])
            imagenames.append(filenamez)
        #/var/www/html/basic/web/downloaded_content
        return ",".join(imagenames)
        # return ""
